...
- (UIImage *)blurredImageWithRadius:(CGFloat)radius iterations:(NSUInteger)iterations tintColor:(UIColor *)tintColor
{
    ...
    {
        CGContextSetFillColorWithColor(ctx, [tintColor colorWithAlphaComponent:0.85].CGColor);
        CGContextSetBlendMode(ctx, kCGBlendModeSourceAtop);// kCGBlendModePlusLighter);
...
- (id<CAAction>)actionForLayer:(CALayer *)layer forKey:(NSString *)key
{
    if ([key isEqualToString:@"blurRadius"])
    {
        ...
        if ( ! [action isKindOfClass:[NSNull class]] ) // if ((NSNull *)action != [NSNull null])
        {
